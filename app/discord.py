...
DEFAULT_DATASET = os.environ["DEFAULT_DATASET"]
...
def _ask_chartgpt(
    ...
    try:
        ...
        agent = analytics_bot_langchain.get_agent(
            dataset_ids=[dataset] if dataset else None
        )
        response = agent(
...
async def ask_chartgpt(
    ...
    embed.set_footer(text="Made with ❤️ by ***REMOVED***")
